...
public class Deck
{
    private final HashMap<Integer, Carta> diccionarioCartas;  //HashMap que almacena la información de todas las cartas de la baraja, incluye una llave de tipo Integer con su contenido, el cual es un objeto de tipo Carta, ESTA COLECCIÓN NO SE MODIFICA
    ...
    public void Shuffle(boolean primerShuffle)   // Se modifica el orden en el que se encuentran las cartas aún restantes en el deck
    {
        ...
        switch(llavesCartas.size())
        {
            ...
            default:    //Hay 0 o mas de 2 cartas en el deck
    ...
    {
        try  //Para efectuar esta acción, el deck debe tener por lo menos una carta dentro de él
        {
        ...
        catch (IndexOutOfBoundsException a)   //No hay cartas, por lo que la acción no se realiza
        {
    ...
    public void Pick()  //Se toma cualquier carta de la lista
    {
        switch(llavesCartas.size())     //Dependiendo del tamaño de la Deck, se hacen las siguientes opciones
        {
            ...
            default:    //Hay 2 o más cartas
                ...
                catch (IllegalArgumentException a)   //No hay cartas, por lo que la acción no se realiza
                {
                    JOptionPane.showMessageDialog(null,"El Deck ya no posee mas cartas", "BARAJA VACIA", JOptionPane.INFORMATION_MESSAGE);
                }
    ...
    {
        try     //Si el deck aun tiene mas de 5 cartas dentro de si disponibles se puede realizar la operación
        {
            ArrayList<Integer> listTemporal = new ArrayList<Integer>();
            ...
            listTemporal.addAll(llavesCartas);
            ...
            for(byte i = 0; i < 5; i++)
            {
                ...
                int aleatorio = r.nextInt(listTemporal.size());
                indicesCartasManos[i] = listTemporal.get(aleatorio);
                manoCartas[i] = diccionarioCartas.get(indicesCartasManos[i]);
                listTemporal.remove(indicesCartasManos[i]);
            }
        ...
        catch(IllegalArgumentException a)    //Si el deck tiene menos de 5 cartas no se realiza nada
        {
